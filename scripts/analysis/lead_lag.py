...
lead-lag (existing analysis/lead_lag.py) only proves the two PM markets share
...
At each lag k ∈ {-10, ..., +10} we pool all valid (c, t) pairs and compute
...
Outputs
-------
data/cleaned/pm_spot_lead_lag.json  — peak lag, peak r, and full per-lag table
    ...
    for asset in ("BTC", "ETH"):
        ...
        for k in res["lags"]:
            print(f"  {k:>4d} {res['r'][k]:>9.4f}  "
                  ...
                  f"{res['n'][k]:>7,}")
        print(f"  peak: lag = {res['peak_lag']} min, r = {res['peak_r']:+.4f}")
