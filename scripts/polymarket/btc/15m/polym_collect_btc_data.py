...
print(f"Tracking BTC 15m markets. Data saving to {csv_filename}")
...
try:
    while True:
        now = int(time.time())
        period = now - (now % 900)
        slug = f"btc-updown-15m-{period}"
        market_start_time = period

        # 1. DETECT MARKET CHANGE
        ...
        # Initialize next_log_time at market start
            ...
            try:
                ...
                if m_data and len(m_data) > 0:
                    ...
                    if market.get('umaResolutionStatus') == 'resolved' or market.get('closed'):
                        ...
                        if outcome:
                            print(f"SUCCESS: Market {p_slug} resolved to {outcome} | Volume: ${volume:,.2f}")
                ...
                if token_ids:
                    ...
                    if bid_data and ask_data:
                        ...
                        if now >= next_log_time and data_points_this_market < 15:
                            # Average all samples collected since last log
                            if probabilities_up:
                                ...
                                # For first point, use market start time; otherwise use scheduled time
